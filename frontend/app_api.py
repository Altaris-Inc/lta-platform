    ...
    if not tape:
        ...
        if dq_key not in st.session_state:
            with st.spinner("Running data quality checks..."):
                try:
                    csv_bytes = None
                    tape_name = tape.get("filename", "tape.csv") if tape else "tape.csv"
                    if st.session_state.get("df") is not None:
                        import io as _io
                        buf = _io.StringIO()
                        st.session_state.df.to_csv(buf, index=False)
                        csv_bytes = buf.getvalue().encode("utf-8")
                    dq = client.get_dq(st.session_state.tape_id, csv_bytes=csv_bytes, filename=tape_name)
                    ...
                    if check_type == "Missing values":
                        ...
                        if domain_rule:
                            ...
                            if isinstance(mn, str):
                                ...
                                mask = dts.notna() & ((dts < mn_dt) | (dts > mx_dt))
                            else:
                                try:
                                    nums = col_series.apply(lambda v: float(str(v).replace(",", "").replace("$", "").strip()) if not _is_null_val(v) else None)  # noqa: E501
                                    mask = nums.notna() & (
                                        ((nums < mn) if mn is not None else pd.Series([False]*len(nums))) |
                                        ((nums > mx) if mx is not None else pd.Series([False]*len(nums)))
                                    )
                                except Exception:
                                    mask = pd.Series([False] * len(df_full))
                        else:
                        ...
                        if not domain_rule2:
                            for pat, mn, mx in _PATTERN_DOMAIN2:
                                if pat.search(col_name):
                                    domain_rule2 = (mn, mx)
                                    break
                        if domain_rule2:
                            mn2, mx2 = domain_rule2
                            if isinstance(mn2, str):
                                from datetime import datetime as _dt2
                                mn_dt2 = _dt2.strptime(mn2, "%Y-%m-%d")
                                mx_dt2 = _dt2.strptime(mx2, "%Y-%m-%d")
                                def _parse_dt2(v):
                                    if _is_null_val(v):
                                        return None
                                    for fmt in ("%d/%m/%Y", "%m/%d/%Y", "%Y-%m-%d", "%d-%m-%Y", "%Y%m%d"):
                                        try:
                                            return _dt2.strptime(str(v).strip(), fmt)
                                        except Exception:
                                            continue
                                    return None
                                dts2 = col_series.apply(_parse_dt2)
                                domain_mask = dts2.notna() & ((dts2 < mn_dt2) | (dts2 > mx_dt2))
                            else:
                                try:
                                    ...
                                    domain_mask = nums2.notna() & (
                                        ((nums2 < mn2) if mn2 is not None else pd.Series([False]*len(nums2))) |
                                        ((nums2 > mx2) if mx2 is not None else pd.Series([False]*len(nums2)))
                                    )
                                except Exception:
                                    pass
...
# ═══════════════════════════════════════════════════════════════
    ...
    _log.warning(f"[MAPPING DEBUG] tape_id={st.session_state.tape_id}, mp={len(mp)}, hdrs={len(hdrs)}, mapped={len(mapped_fields)}, unmapped_ref={len(unmapped_ref)}")  # noqa: E501
    for _k, _v in mp.items():
        _in_hdrs = _v in hdrs
        _in_mapped = _k in mapped_fields
        if not _in_hdrs or not _in_mapped:
            _log.warning(f"[MAPPING DEBUG] ISSUE: field={_k}, col={_v}, in_hdrs={_in_hdrs}, in_mapped={_in_mapped}")  # noqa: E501
    ...
    if derived_cols:
        with st.expander(f"⚗️ Derived Period Columns ({len(derived_cols)})", expanded=True):
            st.markdown(
                ...
                'These columns were auto-generated by differencing cumulative fields within each loan. '
                'They are available in your exported CSV and can be mapped to standard fields below.'
                '</span>',
                unsafe_allow_html=True
            )
            ...
            for i, col_name in enumerate(sorted(derived_cols)):
                ...
                src_candidates = [
                    ...
                    if base in h.lower() and any(
                        p in h.lower() for p in ["cum", "cumulative", "ytd", "accum", "_to_date"]
                    )
                ]
                src_label = f" ← {src_candidates[0]}" if src_candidates else ""

                container = col_d1 if i % 2 == 0 else col_d2
                with container:
                    st.markdown(
                        f'<div style="background:#171C24;border:1px solid #2A3545;border-radius:8px;'
                        f'padding:10px 14px;margin-bottom:8px;">'
                        f'<span style="color:#00D4AA;font-size:12px;font-weight:600">⚗️ {col_name}</span>'
                        f'<span style="color:#566375;font-size:10px">{src_label}</span>'
                        f'</div>',
                        unsafe_allow_html=True
                    )
                    ...
                    map_sel = st.selectbox(
                        ...
                        map_options,
                        key=f"derived_{col_name}",
                        label_visibility="collapsed"
                    )
                    if map_sel != "— (keep as-is) —":
                        new_mp[map_sel] = col_name
                        changed = True
